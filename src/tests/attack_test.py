...
def test_resolve_hit():
    attack = AttackBuilder(d(10)).prof(3).amod(3).adv().gwm().attbon(3).dmgbon(2).build()
    assert 0.6525 == attack.resolve_hit(15)

def test_lucky_hit():
    attack = AttackBuilder(d(1)).adv().lucky().build()
    hit_outcome = attack.resolve_hit(15)
