...
class IntegerInterval(Pmf[int]):
    def __init__(self, probabilities: List[float], offset: int, pmf_factory: IPmfFactory):
    ...
    @classmethod
    def from_table(cls, table: Dict[int, float], pmf_factory: IPmfFactory) -> "IntegerInterval":
    ...
    def __add_helper(self, other):
        ...
        for s in range(self.size):
            ...
            for i in range(s + 1):
                probabilities[s] += self.probabilities[i] * other.probabilities[s - i]
