...
class TurnBuilder:
    ...
    def attack(self, attack: Union[Attack, AttackBuilder], times: int = 1):
        self.attacks.extend(itertools.repeat(attack
                                             if isinstance(attack, Attack)
                                             else attack.build(), times))
        return self

    def resolve_turn_extra_damage(self, hit_outcome: HitOutcome) -> Pmf[int]:
        if hit_outcome == HitOutcome.CRITICAL_HIT:
            return pmf.to_pmf(self.turn_damage_roll).times(2) + \
                self.turn_damage_bonus
    ...
    def resolve(self, armor_class: IntDist) -> Pmf[int]:
        attack_outcomes = [attack.resolve(armor_class)
                           for attack in self.attacks]
        hardest_hit: Pmf[HitOutcome] = functools.reduce(max, map(
            lambda outcome: outcome.hit_outcome, attack_outcomes))
        total_damage = \
            sum(map(lambda outcome: outcome.damage, attack_outcomes)) + \
