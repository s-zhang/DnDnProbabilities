...
class Attack:
    ...
    def resolve_hit(self, armor_class: Pmf[int]) -> Pmf[HitOutcome]:
        def resolve_hit_helper(roll: int) \
                -> Union[HitOutcome, Pmf[HitOutcome]]:
            if roll >= self.critical_threshold:
                return HitOutcome.CRITICAL_HIT
            elif roll == 1:
                return HitOutcome.CRITICAL_MISS
            else:
                return pmf.if_(roll + self.attack_bonus >= armor_class,
                               HitOutcome.NORMAL_HIT, HitOutcome.NORMAL_MISS)
    ...
    def resolve(self, armor_class: Pmf[int]) -> Pmf[AttackOutcome]:
        ...
        def resolve_attack_outcome(hit_outcome: HitOutcome)\
                -> Pmf[AttackOutcome]:
            damage_dist = resolve_attack_damage(hit_outcome)
            return damage_dist.map_pmf(
                lambda damage: AttackOutcome(hit_outcome, damage))
